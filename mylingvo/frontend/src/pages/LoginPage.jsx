...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Хатолик юз берди');
    }
    setLoading(false);
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: 'var(--bg-dark)' }}>
      <div className="w-full max-w-md rounded-2xl p-8" style={{ background: 'var(--bg-card)' }}>
        ...
        <p className="text-center mb-8" style={{ color: 'var(--text-secondary)' }}>Рус тилини ўрганинг</p>
        ...
        <p className="text-center mt-6 text-sm" style={{ color: 'var(--text-secondary)' }}>
          Аккаунтингиз йўқми?{' '}
          <Link to="/register" className="font-medium" style={{ color: 'var(--primary-light)' }}>
            Рўйхатдан ўтиш
          </Link>
        </p>
      </div>
